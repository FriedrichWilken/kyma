...
var _ = Describe("Subscription Reconciliation Tests", func() {
	...
	When("Creating a Subscription with empty protocol, protocolsettings and dialect", func() {
		It("Should reconcile the Subscription", func() {
			...
			reconcilertesting.SetValidSink(namespaceName, subscriberSvc.Name, givenSubscription)
			ensureSubscriptionCreated(ctx, givenSubscription)
	...
	When("Subscription sink name is changed", func() {
		It("Should update the BEB subscription webhookURL by creating a new APIRule", func() {
			...
			reconcilertesting.SetValidSink(serviceOld.Namespace, serviceOld.Name, givenSubscription)
			...
			reconcilertesting.SetValidSink(serviceNew.Namespace, serviceNew.Name, readySubscription)
	...
	When("BEB subscription creation failed", func() {
		It("Should not mark the subscription as ready", func() {
			...
			// Create subscription
			givenSubscription := reconcilertesting.NewSubscription(subscriptionName, namespaceName, reconcilertesting.WithWebhookAuthForBEB, reconcilertesting.WithEventTypeFilter)
			reconcilertesting.SetValidSink(subscriberSvc.Namespace, subscriberSvc.Name, givenSubscription)
			ensureSubscriptionCreated(ctx, givenSubscription)
		...
		It("Should recreate BEB Subscription again", func() {
			...
			Context("Setup Kyma Subscription required resources", func() {
				...
				By("Creating Kyma Subscription", func() {
					reconcilertesting.SetValidSink(svc.Namespace, svc.Name, kymaSubscription)
